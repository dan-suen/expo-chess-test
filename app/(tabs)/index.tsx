...
import {
  ...
  getHighlightIndices,
  executeCastling,
} from '../hooks/getHighlightIndices';
...
function App() {
  const [positions, setPositions] =
    useState<(string | null)[]>(initialPositions);
  ...
  const [promotingPawnType, setPromotingPawnType] = useState<string | null>(
    null
  );
  const [promotionColor, setPromotionColor] = useState<'white' | 'black'>(
    'white'
  );
  ...
  const handlePromotionSelect = (choice: string) => {
    ...
    if (promotionSquare !== null && activePieceType) {
      const basePieceName = activePieceType.startsWith('W')
        ? `W${choice}`
        : `B${choice}`;
      ...
      setTimeout(() => {
        ...
        console.log(
          'Promoting pawn type before calling findTaken:',
          promotingPawnType
        );
      ...
      console.log(
        'Promotion failed: promotionSquare or activePieceType is null.'
      );
    }
  };
  ...
  const isValidMove = (
    ...
    console.log(
      `Validating move for ${normalizedPieceType} from ${from} to ${to}`
    );
    console.log(
      `Target piece: ${targetPiece}, Row difference: ${rowDiff}, Column difference: ${colDiff}`
    );
      ...
      case 'BB': // Black Bishop
        if (rowDiff === colDiff) {
          ...
          for (let i = 1; i < rowDiff; i++) {
            ...
            if (positions[intermediateRow * 8 + intermediateCol] !== null)
    ...
    (
      ...
      if (
        ...
        if (activeId !== null) {
          ...
          console.log(
            `Attempting to move ${activePieceType} from ${previousId} to ${clickedId}`
          );
  ...
  const clickFunctionEmpty = useCallback(
    (
      ...
      setLastMove: React.Dispatch<
        React.SetStateAction<{ from: number; to: number; piece: string } | null>
      >,
      ...
      if (highlightedSquares.has(currentId) && activeId && activePieceType) {
        ...
        if (
          (activePieceType.startsWith('WP') ||
            activePieceType.startsWith('BP')) && // Must be a pawn
          ...
          if (capturedPawn) {
            ...
            console.log(
              'Captured pawn removed from board at index:',
              enPassantCapturedPawnIndex
            );
            ...
            console.warn(
              "No pawn found to capture via 'en passant' at index:",
              enPassantCapturedPawnIndex
            );
          }
        } else {
        ...
        if (
          ...
          setPromotionColor(
            activePieceType.startsWith('W') ? 'white' : 'black'
          );
        ...
        if (
          targetPiece &&
          ((turn === 'white' && targetPiece.startsWith('W')) ||
            (turn === 'black' && targetPiece.startsWith('B')))
        ) {
  ...
  const memoizedRefs = useMemo(
    ...
    [
      refs.WP1Ref.current,
      refs.WP2Ref.current,
      refs.WP3Ref.current /* include all refs here */,
    ]
  );
  ...
  return (
    <div className="App">
      ...
      <table id="chessboard">{/* Render chessboard here */}</table>
    </div>
  );
}
